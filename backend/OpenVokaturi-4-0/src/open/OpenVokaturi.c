...
const char *Vokaturi_versionAndLicense () {
	return
		"OpenVokaturi version 4.0 for open-source projects, 2022-08-22\n"
		"Distributed under the GNU General Public License, version 3 or later";
}

struct structVokaturiVoice {
	atomic_int_least64_t numberOfFedSamples;
	double sampleRate;
	bool multiThreading;
	int bufferLength;
	double *buffer;
	flint numberOfAnalyzedSamples;
...
VokaturiVoice VokaturiVoice_create (double sampleRate, int bufferLength, int multiThreading) {
	...
	my multiThreading = multiThreading;
	...
	*/
	...
	if (multiThreading)
		(void) atomic_fetch_add (& my numberOfFedSamples, (int_least64_t) 0);
	return me;
error:
...
void VokaturiVoice_fill (VokaturiVoice me, int numberOfSamples, double samples []) {
	...
	my numberOfFedSamples += numberOfSamples;
	//fprintf (stderr, "samples fed %ld analyzed %ld\n", (long) voice -> numberOfFedSamples, (long) voice -> numberOfAnalyzedSamples);
}

void VokaturiVoice_fill_float64array (VokaturiVoice me, int numberOfSamples, double samples []) {
	int buffer_index = flint_mod (my numberOfFedSamples, my bufferLength);
	for (int isamp = 0; isamp < numberOfSamples; isamp ++) {
		my buffer [buffer_index] = samples [isamp];
		buffer_index ++;
	...
	if (my multiThreading) {
		atomic_fetch_add (& my numberOfFedSamples, (int_least64_t) numberOfSamples);
	} else {
		my numberOfFedSamples += numberOfSamples;
	}
}

void VokaturiVoice_fill_float32array (VokaturiVoice me, int numberOfSamples, float samples []) {
	int buffer_index = flint_mod (my numberOfFedSamples, my bufferLength);
	for (int isamp = 0; isamp < numberOfSamples; isamp ++) {
		my buffer [buffer_index] = samples [isamp];
		buffer_index ++;
...
void VokaturiVoice_fill_int32array (VokaturiVoice me, int numberOfSamples, int samples []) {
	int buffer_index = flint_mod (my numberOfFedSamples, my bufferLength);
	for (int isamp = 0; isamp < numberOfSamples; isamp ++) {
		my buffer [buffer_index] = samples [isamp] / 2147483648.0;
		buffer_index ++;
		if (buffer_index == my bufferLength)
			buffer_index = 0;
	}
...
void VokaturiVoice_fill_int16array (VokaturiVoice me, int numberOfSamples, short samples []) {
	int buffer_index = flint_mod (my numberOfFedSamples, my bufferLength);
	for (int isamp = 0; isamp < numberOfSamples; isamp ++) {
		my buffer [buffer_index] = samples [isamp] / 32768.0;
		buffer_index ++;
		if (buffer_index == my bufferLength)
			buffer_index = 0;
	}
...
void VokaturiVoice_fill_float64value (VokaturiVoice me, double sample) {
	int buffer_index = flint_mod (my numberOfFedSamples, my bufferLength);
	my buffer [buffer_index] = sample;
	if (my multiThreading) {
		atomic_fetch_add (& my numberOfFedSamples, (int_least64_t) 1);
	} else {
		my numberOfFedSamples += 1;
	}
}

void VokaturiVoice_fill_float32value (VokaturiVoice me, float sample) {
	int buffer_index = flint_mod (my numberOfFedSamples, my bufferLength);
	my buffer [buffer_index] = sample;
	if (my multiThreading) {
		atomic_fetch_add (& my numberOfFedSamples, (int_least64_t) 1);
...
void VokaturiVoice_fill_int32value (VokaturiVoice me, int sample) {
	int buffer_index = flint_mod (my numberOfFedSamples, my bufferLength);
	my buffer [buffer_index] = sample / 2147483648.0;
	if (my multiThreading) {
		atomic_fetch_add (& my numberOfFedSamples, (int_least64_t) 1);
	} else {
		my numberOfFedSamples += 1;
...
void VokaturiVoice_fill_int16value (VokaturiVoice me, int sample) {
	int buffer_index = flint_mod (my numberOfFedSamples, my bufferLength);
	my buffer [buffer_index] = sample / 32768.0;
	if (my multiThreading) {
		atomic_fetch_add (& my numberOfFedSamples, (int_least64_t) 1);
	} else {
		my numberOfFedSamples += 1;
...
void VokaturiVoice_fillInterlacedStereo_float64array (VokaturiVoice me, VokaturiVoice you,
	int numberOfSamplesPerChannel, double samples [])
{
	int my_buffer_index   = flint_mod (my   numberOfFedSamples, my   bufferLength);
	int your_buffer_index = flint_mod (your numberOfFedSamples, your bufferLength);
	for (int isamp = 0; isamp < numberOfSamplesPerChannel; isamp ++) {
		my   buffer [my_buffer_index]   = samples [2 * isamp];
		your buffer [your_buffer_index] = samples [2 * isamp + 1];
		my_buffer_index ++;
		if (my_buffer_index == my bufferLength)
			my_buffer_index = 0;
		your_buffer_index ++;
		if (your_buffer_index == your bufferLength)
		your_buffer_index = 0;
	}
	if (my multiThreading) {
		atomic_fetch_add (& my   numberOfFedSamples, (int_least64_t) numberOfSamplesPerChannel);
		atomic_fetch_add (& your numberOfFedSamples, (int_least64_t) numberOfSamplesPerChannel);
	} else {
		my   numberOfFedSamples += numberOfSamplesPerChannel;
		your numberOfFedSamples += numberOfSamplesPerChannel;
	}
}

void VokaturiVoice_fillInterlacedStereo_float32array (VokaturiVoice me, VokaturiVoice you,
	int numberOfSamplesPerChannel, float samples [])
{
	int my_buffer_index   = flint_mod (my   numberOfFedSamples, my   bufferLength);
	int your_buffer_index = flint_mod (your numberOfFedSamples, your bufferLength);
	for (int isamp = 0; isamp < numberOfSamplesPerChannel; isamp ++) {
...
void VokaturiVoice_fillInterlacedStereo_int32array (VokaturiVoice me, VokaturiVoice you,
	int numberOfSamplesPerChannel, int samples [])
{
	int my_buffer_index   = flint_mod (my   numberOfFedSamples, my   bufferLength);
	int your_buffer_index = flint_mod (your numberOfFedSamples, your bufferLength);
	for (int isamp = 0; isamp < numberOfSamplesPerChannel; isamp ++) {
		my   buffer [my_buffer_index]   = samples [2 * isamp] / 2147483648.0;
		your buffer [your_buffer_index] = samples [2 * isamp + 1] / 2147483648.0;
		my_buffer_index ++;
		if (my_buffer_index == my bufferLength)
			my_buffer_index = 0;
		your_buffer_index ++;
...
void VokaturiVoice_fillInterlacedStereo_int16array (VokaturiVoice me, VokaturiVoice you,
	int numberOfSamplesPerChannel, short samples [])
{
	int my_buffer_index   = flint_mod (my   numberOfFedSamples, my   bufferLength);
	int your_buffer_index = flint_mod (your numberOfFedSamples, your bufferLength);
	for (int isamp = 0; isamp < numberOfSamplesPerChannel; isamp ++) {
		my   buffer [my_buffer_index]   = samples [2 * isamp] / 32768.0;
		your buffer [your_buffer_index] = samples [2 * isamp + 1] / 32768.0;
		my_buffer_index ++;
		if (my_buffer_index == my bufferLength)
			my_buffer_index = 0;
		your_buffer_index ++;
...
void VokaturiVoice_extractCues9 (VokaturiVoice me, VokaturiQuality *quality, CueStrengths9 cueStrengths) {
	...
	*/
	const flint numberOfFedSamples =
		my multiThreading ?
		(int64_t) atomic_fetch_add (& my numberOfFedSamples, (int_least64_t) 0) :
		(int64_t) my numberOfFedSamples
	;
	if (numberOfFedSamples < my frameLength)
		return VokaturiQuality_error (quality);

	flint oldNumberOfAnalyzedFrames = my numberOfAnalyzedFrames;
	flint newNumberOfAnalyzedFrames = flint_div (numberOfFedSamples - my frameLength, my frameStep) + 1;
	...
	//assert (newNumberOfAnalyzedSamples <= numberOfFedSamples);
	...
	for (int iframe = 0; iframe < numberOfFramesToAnalyze; iframe ++) {
		...
		*/
		...
		)
		{
			...
			dpitch1 = real_mod (dpitch1, 12.0);
			if (dpitch1 > 6.0)
				dpitch1 = 12.0 - dpitch1;
			double dpitch2 = fabs (my right.pitch - my mid.pitch);
			dpitch2 = real_mod (dpitch2, 12.0);
			if (dpitch2 > 6.0)
				dpitch2 = 12.0 - dpitch2;
